        ...
        <div className="space-y-6 max-w-[1600px] w-full mx-auto">
          ...
          {sidebarTab === 'problem-statements' && (
            <div className="space-y-6 animate-in fade-in duration-200">
              ...
              {/* SIH Official Problem Statements Card */}
              <div className="rounded-2xl border border-[#1B3F8B]/20 bg-gradient-to-br from-[#1B3F8B]/5 via-blue-50/60 to-white p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4 shadow-sm">
                <div className="flex-shrink-0 w-12 h-12 rounded-2xl bg-[#1B3F8B] flex items-center justify-center shadow-md">
                  <ExternalLink className="h-6 w-6 text-white" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <h3 className="text-sm font-black text-[#1B3F8B]">SIH 2026 Official Problem Statements</h3>
                  ...
                  <p className="text-xs text-slate-600 font-medium leading-relaxed">
                    Browse all 226 official SIH 2026 Problem Statements on the Government of India portal. Add the ones relevant to VSITR using <strong>"Add Problem Statement"</strong> button above.
                  </p>
                  <p className="text-[11px] text-slate-400 font-mono mt-1">https://www.sih.gov.in/sih2026PS</p>
                </div>
                <a
                  href="https://www.sih.gov.in/sih2026PS"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl font-extrabold text-xs text-white bg-[#1B3F8B] hover:bg-blue-900 shadow-md transition shrink-0"
                >
                  <ExternalLink className="h-3.5 w-3.5" />
                  Visit SIH Official Website
                </a>
              </div>
                    ...
                    <table className="w-full text-xs border-collapse min-w-[800px]">
                      <thead className="bg-slate-50/90 border-b border-slate-200/80">
                        <tr className="text-left font-extrabold text-slate-500 uppercase tracking-wider whitespace-nowrap">
                          ...
                          <th className="py-3.5 px-4 text-center">Selected By</th>
                          <th className="py-3.5 px-4 text-right">Actions</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 font-medium text-slate-800">
                        {adminPsList.map((ps) => {
                          const selectedTeams = teams.filter(t => t.selectedPsId === ps.id);
                          return (
                            <tr key={ps.id} className="hover:bg-slate-50/80 transition-colors">
                              ...
                              <td className="py-3.5 px-4 text-center whitespace-nowrap">
                                {selectedTeams.length > 0 ? (
                                  <span
                                    title={selectedTeams.map(t => `${t.id} — ${t.teamName}`).join('\n')}
                                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-extrabold bg-blue-50 text-[#1B3F8B] border border-blue-200 cursor-default"
                                  >
