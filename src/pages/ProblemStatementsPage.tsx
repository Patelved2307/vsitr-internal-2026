...
import { BookOpen, AlertTriangle, Check, AlertCircle, ExternalLink, Globe } from 'lucide-react';
...
export const ProblemStatementsPage: React.FC = () => {
  ...
  // Use admin-controlled deadline; fall back to false so selection isn't blocked by default
  const psDeadline = settings?.psSelectionDeadline ? new Date(settings.psSelectionDeadline) : null;
  const isPsExpired = psDeadline ? Date.now() > psDeadline.getTime() : false;
  ...
  return (
    <div className="py-8 px-4 max-w-[1200px] mx-auto space-y-8 animate-in fade-in duration-300">
      ...
      {/* SIH Official Problem Statements Card — Always Visible */}
      <div className="rounded-2xl border border-[#1B3F8B]/25 bg-gradient-to-br from-[#1B3F8B]/8 via-blue-50/70 to-white p-5 sm:p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5 shadow-sm">
        <div className="flex-shrink-0 w-14 h-14 rounded-2xl bg-[#1B3F8B] flex items-center justify-center shadow-lg">
          <Globe className="h-7 w-7 text-white" />
        </div>
        <div className="flex-1 min-w-0 space-y-1">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-base font-black text-[#1B3F8B]">SIH 2026 Official Problem Statements</h3>
            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-extrabold bg-[#C1272D] text-white uppercase tracking-wider">226 PS Available</span>
          </div>
          <p className="text-xs text-slate-600 font-medium leading-relaxed">
            Browse all <strong>226 official SIH 2026 Problem Statements</strong> released by the Government of India. Your institute may add specific ones below for selection. You can also view the complete list on the official portal.
          </p>
          <p className="text-[11px] text-slate-400 font-mono">sih.gov.in/sih2026PS</p>
        </div>
        <a
          href="https://www.sih.gov.in/sih2026PS"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 px-5 py-3 rounded-2xl font-extrabold text-sm text-white bg-[#1B3F8B] hover:bg-blue-900 shadow-md transition shrink-0 self-stretch sm:self-auto justify-center"
        >
      ...
      {/* Institute Problem Statements Section */}
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <h2 className="text-lg font-black text-slate-900">Institute Problem Statements</h2>
          {!isLoading && problemStatements.length > 0 && (
            <span className="px-2.5 py-0.5 rounded-full text-xs font-extrabold bg-slate-100 text-slate-600 border border-slate-200">
        ...
        {isLoading ? (
          <div className="space-y-3">
            ...
            <div className="h-40 w-full bg-slate-100 animate-pulse rounded-2xl" />
          </div>
        ) : error ? (
          ...
          <div className="p-10 text-center space-y-4 bg-white rounded-3xl border border-slate-200 shadow-sm">
            ...
            <div className="space-y-1">
              <h3 className="font-black text-slate-800 text-base">No institute problem statements yet</h3>
              <p className="text-sm text-slate-500 font-medium leading-relaxed max-w-sm mx-auto">
                Your institute hasn't released specific problem statements yet. Use the official SIH portal above to browse all 226 available problem statements.
              </p>
            </div>
            <a
              href="https://www.sih.gov.in/sih2026PS"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-sm text-[#1B3F8B] bg-blue-50 hover:bg-blue-100 border border-blue-200 transition"
            >
              <ExternalLink className="h-4 w-4" />
              Browse on SIH Official Website
            </a>
          </div>
        ) : (
          <div className="space-y-4">
