...
export const RegistrationRulesView: React.FC = () => {
  ...
  useEffect(() => {
    const calculatePptTimeLeft = () => {
      const pptDeadlineStr = settings?.pptSubmissionDeadline || '2026-09-08T23:59:00+05:30';
      const pptDeadline = new Date(pptDeadlineStr).getTime();
      const nowVal = new Date().getTime();
              ...
              {timeline && timeline.map((event, idx) => {
                ...
                return (
                  <div key={event.id || idx} className={`flex flex-col sm:flex-row sm:items-center justify-between gap-2 p-3 rounded-2xl border transition ${
                    ...
                    <span className="text-xs font-extrabold text-slate-800 leading-snug flex-1" title={event.title}>
                      {event.title}
                    </span>
                    <span className={`font-black text-xs self-start sm:self-auto px-2.5 py-1 rounded-xl text-right max-w-[180px] sm:max-w-[220px] leading-snug ${
                      isClosed
                      ...
                      {isLivePpt && <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />}
                      {displayDate}
                    </span>
                  </div>
                );
