...
import { EventSettings, FAQItem, Team, TimelineEvent, EmailLog, Rule, ProblemStatement, PptSubmission } from '../types.js';
...
// Match the application server: support the documented `.env.local` file,
// with `.env` as a deployment fallback. Never load `.env.example`.
...
export async function getGlobalConfig() {
  if (isNeonConnected) {
    try {
      ...
      if (res.rows.length > 0) {
        ...
        if (!mergedSettings.pptSubmissionDeadline || mergedSettings.pptSubmissionDeadline === '25 August 2026, 11:59 PM' || mergedSettings.pptSubmissionDeadline.includes('23 August') || !/^\d{4}-\d{2}-\d{2}T/.test(mergedSettings.pptSubmissionDeadline)) {
          mergedSettings.pptSubmissionDeadline = '2026-08-24T18:30:00.000Z';
        }
        const mergedTimeline = val.timeline || fileDb.timeline;
        return {
  ...
  const fileDb = ensureFileDb();
  if (!fileDb.settings.pptSubmissionDeadline || fileDb.settings.pptSubmissionDeadline === '25 August 2026, 11:59 PM' || fileDb.settings.pptSubmissionDeadline.includes('23 August') || !/^\d{4}-\d{2}-\d{2}T/.test(fileDb.settings.pptSubmissionDeadline)) {
...
export async function updateTeamPsSelection(teamId: string, psId: string | null, psTitle: string | null): Promise<Team | null> {
  ...
  const cleanPsTitle = (psTitle || '').trim();
  const team = await getTeamById(cleanTeamId);
  if (!team) return null;

  if (!cleanPsId || !cleanPsTitle) {
    throw new Error('A problem statement ID and title are required.');
  }

  const updatedFields = {
    selectedPsId: cleanPsId,
    selectedPsTitle: cleanPsTitle,
    psSelectedAt: new Date().toISOString(),
  };

  const updatedTeam: Team = { ...team, ...updatedFields, updatedAt: new Date().toISOString() };

  // Do not report a successful selection until Neon has confirmed the write. The
  // generic team updater intentionally supports a local fallback, which can make
  // a serverless instance appear successful while the admin panel still reads an
  // unchanged Neon record.
  if (isNeonConnected) {
    try {
      const result = await runQuery(
        `UPDATE teams
         SET selected_ps_id = $1, selected_ps_title = $2, ps_selected_at = $3, updated_at = $4
         WHERE UPPER(id) = UPPER($5)
         RETURNING id`,
        [
          updatedTeam.selectedPsId,
          updatedTeam.selectedPsTitle,
          updatedTeam.psSelectedAt,
      ...
      console.error('Error saving problem statement selection in Neon DB:', err);
      throw new Error('Could not save the problem statement selection to the database. Please try again.');
    }
  }

  // Keep the local JSON mirror in sync for development and as a read fallback.
  const db = ensureFileDb();
  const index = db.teams.findIndex((entry) => entry.id.toUpperCase() === cleanTeamId.toUpperCase());
  if (index !== -1) {
