...
export const api = {
  ...
  // Admin: Get all team IDs for autocomplete (Team Lookup)
  getAllTeamIds: async () => {
    const res = await fetch('/api/admin/all-team-ids');
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Failed to fetch team IDs');
    return data as { success: boolean; teamIds: string[] };
  },

  // Admin: Get distinct mentor names for autocomplete (Mentor Lookup)
  getAllMentorNames: async () => {
    const res = await fetch('/api/admin/all-mentor-names');
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Failed to fetch mentor names');
    return data as { success: boolean; mentorNames: string[] };
  },

  // Admin: Get full team record by ID (Team Lookup detail)
  getAdminTeamById: async (teamId: string) => {
    const res = await fetch(`/api/admin/team/${encodeURIComponent(teamId)}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Team not found');
    return data;
  },

  // Admin: Get all teams for a mentor name (Mentor Lookup detail)
  getAdminMentorTeams: async (mentorName: string) => {
    const res = await fetch(`/api/admin/mentor/${encodeURIComponent(mentorName)}`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'No teams found for mentor');
    return data as {
      success: boolean;
      mentorName: string;
