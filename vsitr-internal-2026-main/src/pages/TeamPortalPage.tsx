...
export const TeamPortalPage: React.FC = () => {
  ...
  const [sihPsOrganization, setSihPsOrganization] = useState('');
  const [sihPsCategory, setSihPsCategory] = useState('');
  const [sihPsTheme, setSihPsTheme] = useState('');
      ...
      <div className="rounded-3xl bg-white p-6 border border-slate-200 shadow-xl space-y-4 hover:shadow-2xl/10 transition duration-300">
          ...
          <div className="flex items-start sm:items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                Select an official SIH 2026 problem statement
              </p>
            </div>
          </div>
        ...
        {team.selectedPsId ? (
            ...
            <div className="rounded-2xl bg-emerald-50 border border-emerald-200 overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    ['Organization', team.selectedPsOrganization],
                    ['Category', team.selectedPsCategory],
                    ['Theme', team.selectedPsTheme],
          ...
          <div className="space-y-4 animate-in fade-in duration-200">
            {!psTimeLeft.isExpired ? (
              <>
                <div className="p-5 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200 shadow-xs">
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-black text-[#1B3F8B] uppercase tracking-wider">Official SIH 2026 Problem Statement</span>
                      </div>
                      <p className="text-xs text-slate-600 font-semibold leading-relaxed pl-7">
                        Browse the official SIH problem statements, then enter all details for your selected statement below.
                      </p>
                    </div>
                    <a
                      ...
                      className="shrink-0 inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl font-black text-xs text-white bg-[#1B3F8B] hover:bg-indigo-900 transition transform active:scale-95 duration-200"
                    >
                      Browse SIH 2026 PS
                      <ExternalLink className="h-3.5 w-3.5" />
                    </a>
                  ...
                  <div className="mt-4 pt-4 border-t border-blue-200">
                    <p className="text-[11px] font-bold text-[#1B3F8B] mb-3">After selecting on the official website, enter all problem statement details here:</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                      <input
                        ...
                        className="px-3 py-2.5 rounded-xl border border-blue-300 bg-white text-xs font-bold text-slate-900 focus:outline-none focus:border-[#1B3F8B] placeholder:font-normal placeholder:text-slate-400"
                      />
                      <input
                        type="text"
                        ...
                        className="px-3 py-2.5 rounded-xl border border-blue-300 bg-white text-xs font-bold text-slate-900 focus:outline-none focus:border-[#1B3F8B] placeholder:font-normal placeholder:text-slate-400"
                      />
                      <input type="text" placeholder="Organization" value={sihPsOrganization} onChange={(e) => setSihPsOrganization(e.target.value)} className="px-3 py-2.5 rounded-xl border border-blue-300 bg-white text-xs font-bold text-slate-900 focus:outline-none focus:border-[#1B3F8B] placeholder:font-normal placeholder:text-slate-400" />
                      <input type="text" placeholder="Category" value={sihPsCategory} onChange={(e) => setSihPsCategory(e.target.value)} className="px-3 py-2.5 rounded-xl border border-blue-300 bg-white text-xs font-bold text-slate-900 focus:outline-none focus:border-[#1B3F8B] placeholder:font-normal placeholder:text-slate-400" />
                      <input type="text" placeholder="Theme" value={sihPsTheme} onChange={(e) => setSihPsTheme(e.target.value)} className="sm:col-span-2 px-3 py-2.5 rounded-xl border border-blue-300 bg-white text-xs font-bold text-slate-900 focus:outline-none focus:border-[#1B3F8B] placeholder:font-normal placeholder:text-slate-400" />
                    </div>
                    <button
                      type="button"
                      disabled={!sihPsNumber.trim() || !sihPsTitle.trim() || !sihPsOrganization.trim() || !sihPsCategory.trim() || !sihPsTheme.trim() || isSubmittingSihPs}
                      onClick={async () => {
                      ...
                      className="mt-3 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-black text-xs text-white bg-[#1B3F8B] hover:bg-indigo-900 transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed transform active:scale-95 duration-200"
                    >
                      {isSubmittingSihPs ? (
