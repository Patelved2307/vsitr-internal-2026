...
app.post('/api/teams/select-ps', async (req: Request, res: Response) => {
  try {
    ...
    const problemStatement = (await getAllProblemStatements()).find((statement) => statement.id === String(psId).trim());
    if (!problemStatement || problemStatement.status !== 'open') {
    ...
    const updatedTeam = await updateTeamPsSelection(teamId, problemStatement.id, problemStatement.title);
    if (!updatedTeam) {
      return res.status(404).json({ success: false, message: 'Team not found.' });
    }
    ...
    try {
      await dispatchPsSelectionEmails(updatedTeam, problemStatement.id, problemStatement.title);
      console.log(`[PS Selection Email] Successfully dispatched confirmation emails for team ${teamId} (${problemStatement.id})`);
    } catch (emailErr) {
